...
process_attribute <- function(stations, dates, data, attribute) {
  ...
  save(data_city, file=sprintf("asos-airports/fr-%s-raw.rdata", attribute))
...
fill_attribute <- function(data_city,attribute) {
  ...
  save(data_city, file=sprintf("asos-airports/fr-%s.rdata", attribute))  
...
fil <- list.files("fr-asos-rdata")
data <- NULL
for (f in fil) {
  filerdata <- sprintf("fr-asos-rdata/%s", f)
  print(filerdata)
  load(filerdata)
